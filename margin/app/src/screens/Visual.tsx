        ...
        <div style={{ display:"flex", gap:24, alignItems:"flex-start" }}>
          ...
          <div style={{ width:268, flex:"0 0 268px", display:"flex", flexDirection:"column", gap:9 }}>
            {visuals.map((v, i) => {
              ...
              return (
                <motion.button key={v.id} onClick={() => setActive(v.id)}
            ...
            <div className="card" style={{ padding:0, overflow:"hidden" }}>
              ...
              <AnimatePresence mode="wait" initial={false}>
                <motion.div key={item.id}
                  ...
                  <div style={{ padding:"20px 22px", borderRight:"1px solid var(--line-2)" }}>
                    ...
                    {item.kind === "figure"
                      ? (item.beforePlot ? <FigurePlot spec={item.beforePlot}/> : <BeforeFigure/>)
                  ...
                  <div style={{ padding:"20px 22px",
                    ...
                    {item.kind === "figure"
                      ? (item.afterPlot ? <FigurePlot spec={item.afterPlot}/> : <AfterFigure/>)
              ...
              <div style={{ padding:"15px 20px", borderTop:"1px solid var(--line-2)",
                ...
                <button className="btn btn-primary btn-sm"
                  onClick={() => navigate(`/paper/${paperId}/reader?note=${item.id}`)}>
...
function PanelLabel({ tone, text }: { tone: "good" | "bad"; text: string }) {
...
function Metric({ tone, text }: { tone: "good" | "bad"; text: string }) {
  const good = tone === "good";
  return (
...
function BeforeText({ text }: { text: string }) {
  return (
    <p style={{ fontFamily:"var(--font-serif)", fontSize:15.5, lineHeight:1.65, margin:0, color:"var(--doc-ink-2)",
      background:"var(--critical-soft)", padding:"14px 15px", borderRadius:10,
      textDecorationLine:"underline", textDecorationColor:"var(--critical)", textDecorationStyle:"wavy",
...
function BeforeFigure() {
  ...
  return (
    <div style={{ position:"relative", background:"var(--surface)", border:"1px solid var(--line)", borderRadius:10,
...
function AfterFigure() {
  const bars = [62, 78, 90, 70];
  return (
    <div style={{ position:"relative", background:"var(--surface)", border:"1px solid var(--ok-soft)", borderRadius:10,
...
function InfoNote({ ic, title, body }: { ic: IconName; title: string; body: string }) {
  return (
